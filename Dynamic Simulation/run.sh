...
PORT="${PORT:-23407}" # HELICS broker port; zmq also opens PORT+1 internally
TARGET_TIME="${TARGET_TIME:-10.0}" # seconds; shared by transmission and distribution simulators
FEEDER_COUNT="${FEEDER_COUNT:-1}"
PROGRESS_INTERVAL="${PROGRESS_INTERVAL:-10}"
BROKER_FEDERATES=$((FEEDER_COUNT + 1))
FINE_DT="${FINE_DT:-0.03}"      # seconds; resolve the bus fault so the feeder sees the voltage sag
COARSE_DT="${COARSE_DT:-0.03}"    # seconds; switch to a larger outer step after transients settle
COARSE_START="${COARSE_START:-1.60}" # seconds; keep this after the fault clear window
HELICS_BROKER_URL="${HELICS_BROKER_URL:-tcp://127.0.0.1:${PORT}}"
DIST_MASTER_DSS="${DIST_MASTER_DSS:-}"

if [[ -z "$DIST_MASTER_DSS" ]]; then
  if [[ -f "8500Node/Master.dss" ]]; then
    DIST_MASTER_DSS="8500Node/Master.dss"
  elif [[ -f "../8500Node/Master.dss" ]]; then
    DIST_MASTER_DSS="../8500Node/Master.dss"
  else
    DIST_MASTER_DSS="8500Node/Master.dss"
  fi
...
if (( FEEDER_COUNT > 0 )) && [[ ! -f "$DIST_MASTER_DSS" ]]; then
  echo "Distribution case not found: $DIST_MASTER_DSS" >&2
  echo "Set DIST_MASTER_DSS to the OpenDSS Master.dss path before running." >&2
  exit 1
fi

PYTHON="${PYTHON:-/home/teng/miniforge3/envs/cosim/bin/python}"
...
wait_for_broker_port() {
  local port="$1"
  local label="$2"

  for i in {1..300}; do
    if ! pid_is_alive "$BROKER_PID"; then
      echo "Broker exited before $label port $port opened." >&2
      sed -n '1,120p' broker.log >&2 || true
      exit 1
    fi
    ss -lnt | grep -q ":$port" && return
    sleep 0.1
  done

  echo "Broker not listening on $label port $port." >&2
  sed -n '1,120p' broker.log >&2 || true
  exit 1
}
...
# Wait for both HELICS zmq sockets. Federates are configured with PORT, while
# HELICS opens PORT+1 internally for core traffic.
wait_for_broker_port "$PORT" "broker"
wait_for_broker_port "$((PORT + 1))" "core"

# Additional delay to ensure broker is fully initialized before federates connect
sleep 0.5
...
echo "  HELICS broker URL: $HELICS_BROKER_URL"
