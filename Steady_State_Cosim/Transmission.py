...
ss.add('Fault', bus=2, tf=1.0, tc=1.10, rf=0.0, xf=0.02)

ss.setup() # Setup the system
ss.PFlow.run() # Run power flow to initialize

TDS_INTERNAL_STEP = 0.005 # Internal time step for TDS
ss.TDS.config.tstep = TDS_INTERNAL_STEP
ss.TDS.config.tf    = 0.0
ss.TDS.run()  # Run TDS to initialize dynamic states
...
target_time = 10.0 # dynamic simulation, change to 10 seconds for testing
...
while current_time < target_time:
    ...
    for k in range(max_outer):
        ...
        ss.TDS.run(no_summary=True) # Run TDS for the current time step without summary to speed up
