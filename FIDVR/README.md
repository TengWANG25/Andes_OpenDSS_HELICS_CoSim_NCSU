...
For the final project video/package, start with
[`PROJECT_WALKTHROUGH.md`](PROJECT_WALKTHROUGH.md). It gives the entry points,
runtime data flow, recommended FIDVR run command, and a short video storyline.
For the WECC Motor D implementation, use
[`WECC_MOTOR_D_TRACEABILITY.md`](WECC_MOTOR_D_TRACEABILITY.md) to map code
blocks and parameters to specific pages in the local WECC report.
...
Motor D model. The old staged surrogate and `IndMach012` experiment paths have
been removed so the code path is easier to audit. Each converted compressor
...
`WECC_MOTOR_D_TRACEABILITY.md` for the page-by-page map.
